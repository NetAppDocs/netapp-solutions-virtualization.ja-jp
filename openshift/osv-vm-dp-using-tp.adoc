...
* link:https://docs.netapp.com/us-en/trident/trident-get-started/quickstart.html["Trident25.02"]
...
[[single-file-restore-for-a-vm]]
== VMの単一ファイル復元

この単一ファイル復元方法は、NFSボリュームなど、クライアントポッド内でファイルシステムとしてマウントされているNAS/ファイルベースのボリュームにのみ適用されます。これは、前述のように `volumeMode: Block`でプロビジョニングされたVMディスクを含む、SAN/rawブロックボリュームには適用されません。

サポートされているNAS/ファイルベースボリュームの場合、特定のファイルを復元する簡単な方法は、バッキングボリューム上のスナップショットディレクトリをクライアントから参照できるようにすることです。これは、Tridentバックエンド構成を編集してデフォルトセクションを追加し、 `snapshotDir`パラメータを `true`に設定することで実行できます。これにより、メインボリュームと同じマウント場所から `.snapshot`ディレクトリへのアクセスが可能になります。 `.snapshot`ディレクトリにはすべてのスナップショットが含まれています。必要なスナップショットディレクトリに移動し、復元したいファイルを見つけて、必要な場所にコピーしてください。


NOTE: この方法では、マウント アクセス権を持つワークロードやユーザーに履歴データが公開されるため、最小特権やコンプライアンスの期待に反する可能性があります。セキュリティや運用への影響を考慮し、この設定は一時的に有効にし、ファイルを復元した後は元に戻す必要があることに注意してください。また、NFS バージョン 4 を使用している場合、 `ls`コマンドでファイルを一覧表示しても `.snapshot`ディレクトリは表示されません。ただし、 `.snapshot`ディレクトリに `cd`することは可能です。

..snapshot ディレクトリにアクセスしてスナップショットからファイルをコピーすることで、単一ファイルを復元する例
[%collapsible]
====
[source, yaml]
----
#single-file-restore.yaml
apiVersion: v1
kind: Pod
metadata:
  name: task-single-file-restore
spec:
  volumes:
    - name: task-pv-storage
      persistentVolumeClaim:
        claimName: test-trident-set-defaults
  containers:
    - name: task-pv-container
      ...
      volumeMounts:
        - mountPath: /data
          name: task-pv-storage
# oc apply -f single-file-restore.yaml
----
image:rh-os-n-use-case-ocpv-tp-dp-sfr-001.png["ポッドとVMのPVC"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-002.png["ONTAP VM ディスクのボリューム名"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-003.png["クライアントでスナップショットディレクトリが表示されます"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-004.png["VM内のマウントディレクトリ"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-005.png["VM内でファイルが失われた場合、スナップショットディレクトリからメインディレクトリにファイルをコピーして復元します。"]
