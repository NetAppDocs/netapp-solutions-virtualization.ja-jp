...
このセクションでは、ONTAP S3 オブジェクトストレージを使用して Trident Protect で AppVault オブジェクトを設定する方法について説明します。

ocコマンドと以下のyamlファイルを使用してシークレットを作成し、ONTAP s3用のAppVaultカスタムリソースを作成します。必ずtrident protect名前空間内に作成してください。
...
image:redhat-openshift-ocpv-tp-002.png["trident-protect名前空間内のOCP-v AppVault"]
