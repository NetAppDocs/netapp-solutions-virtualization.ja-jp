...
* VMDKは複数の方法で構成できます。すべてのVMDKを単一のボリュームに配置する（ストレージ vMotion が不要になる）、各VMDKを個別のボリュームに配置する、または複数のVMDKをボリューム内のqtreeにまとめるといった方法があります。
+

NOTE: Shift ツールキットは、レイアウトを自動的に検出し、適切なクローン作成方法と NAS ストレージドライバを選択します。
