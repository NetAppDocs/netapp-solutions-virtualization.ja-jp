...
[role="lead"]
Shift Toolkit で変換する前に、VMware Storage vMotion と Hyper-V Storage Live Migration を使用して、SAN ベースのデータストアから NAS に VM を移行し、ビジネスの継続性を維持します。
...
VMがNFSデータストアに配置された後、Shift ToolkitはNetApp FlexCloneテクノロジーを使用してVMをVMware ESXiからHyper-Vに変換します。変換されたVMとそのディスクは、Hyper-Vホストからアクセス可能なCIFS共有に配置されます。

.SAN（Hyper-V 環境）への移行*
変換後、Hyper-V ストレージ ライブ マイグレーションを使用して、変換された VM とそのディスクを CIFS 共有から SAN 対応のクラスター共有ボリューム (CSV) に移動します。これにより、Hyper-V 環境で SAN インフラストラクチャを維持できます。
