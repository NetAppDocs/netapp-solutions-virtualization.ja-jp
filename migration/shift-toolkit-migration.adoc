...
* link:shift-toolkit-migrate-esxi2osvmtv.html["Migration Toolkit for Virtualization（MTV）を使用したVMware ESXiからRed Hat OpenShift Virtualizationへの移行"]
* link:shift-toolkit-migrate-esxi2proxmox.html["VMware ESXi から Proxmox VE へ"]
...
一度も実行されていないブループリントは、UIから直接削除できます。ただし、ブループリントがトリガーされると、ジョブ履歴レコードが作成されます。ブループリント自体を削除する前に、これらのジョブ履歴レコードを削除する必要があります。PowerShellの削除スクリプトを使用してこれらのジョブレコードをクリーンアップした後、ブループリントとその関連リソースグループを削除できます。

.手順
. Shift Toolkit UI の *[設定] > [開発者アクセス] > [スクリプトブロック]* に移動して、削除スクリプトをダウンロードしてください。
+
image::shift-toolkit-082.png[設計図名に基づいてジョブを削除する]

. zipフォルダを解凍してください。パッケージには2つのファイルが含まれています。
+
** `removeBpJobs.json` - 設定ファイル
** `removeBpJobs.ps1` - PowerShellスクリプト


.  `removeBpJobs.json`ファイルを編集して、以下を指定します：
+
** この操作の実行名
** Shift ToolkitのIPアドレス
** 管理者認証情報
** 削除するブループリント名
+
設定例：
...
"executions": [
  {
    "execution_name": "reslore",
    "shift_server_ip": "https://10.63.184.75",
    "shift_username": "admin",
    "shift_password": "Admin",
    "blueprint_details": [{"BPTHIMFAT": "True"}]
  }
]
----


. PowerShell を開き、展開したファイルが含まれているフォルダに移動します。
. スクリプトを実行します。
+
[listing]
----
...
スクリプトが正常に完了したら、Shift Toolkit の UI からブループリントとその関連リソースグループを削除できます。
