...
計画されたメンテナンス期間中は、VM の電源を正常にオフにする必要があります。移行を開始する前に、すべての前提条件が満たされていることを確認してください。

* Shift Toolkit VMがドメインの一部であることを確認する
* CIFS共有が適切な権限で設定されていることを確認する
* 移行または変換に使用されるqtreeのセキュリティスタイルが正しいことを確認します
* クラスタ内の任意の Hyper-V ホストから Hyper-V マネージャーを使用して VM の作成をテストし、VHDx を CIFS 共有に配置します。
* プログラムと機能または PowerShell を使用して Hyper-V 管理ツールを追加し、Shift Toolkit VM からの VM の作成をテストします。 `add-windowsfeature rsat-hyper-v-tools`
