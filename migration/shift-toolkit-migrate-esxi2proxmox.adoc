...
* Windows VMの場合、VirtIO ISOがマウントされていることを確認してください。マウントされていない場合、準備プロセスは失敗します。VirtIO ISOドライバはlink:https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/archive-virtio/virtio-win-0.1.271-1/virtio-win-0.1.271.iso["ここをクリックしてください。"]からダウンロードできます。このスクリプトはマウントされたドライブを検出し、必要なファイルを自動的にコピーします。
+
** 準備スクリプトは.msiパッケージを使用してドライバとqemu-guest-agentをインストールするため、リンク先で指定されているISOイメージを使用していることを確認してください。前提条件が整ったら、ShiftツールキットUIにログインし、Proxmox VEを宛先ハイパーバイザーとしてサイトを構成します。追加するには、「新しいサイトを追加」をクリックし、「宛先」を選択します。
...
image::shift-toolkit-469.png[VMディスクイメージがProxmox VE NFSストレージプールにアップロードされました]
